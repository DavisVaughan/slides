...
--
  ...
  - Lot's of time banging my head against the wall
...
- What I've used R for so far at Lincoln
...
- Currently used for _data analysis_
    ...
    - [Liberty Mutual](https://www.rstudio.com/resources/customer-spotlight/liberty-mutual-story/)
...
# The dataset was included in one of the packages
data(FANG)

# This code creates the graph
FANG %>%
  # These lines manipulate data into the right form
  select(date, symbol, adjusted) %>%
  spread(key = symbol, value = adjusted) %>%
  as_xts(date_col = date) %>%
  # These two lines actually graph it
  dygraph() %>%
  dyLegend(width = 600)
```
...
The best part? I can recreate this with new data whenever I feel like, just by switching out "FANG" for another similarly formatted data set. 
...
--
  ...
  - _Challenge:_ 
      ...
      - Create an automated process to aggregate _yearly_ book value and yield data for each portfolio, then display those results in a way to compare this set of 50 year forecasts to previous sets to make sure nothing drastic has changed. 
...
- _New process - 2 R scripts:_
  ...
  - R script 2:
      ...
      - Compare this set of results to previous year's
...
--
  ...
  - _Current process:_
      ...
      - Cannot use the IRR function in Excel. We want to change to a negative discount rate if cumulative cash flows drop below 0.
      
      - __Takes 5-10 minutes to run__
