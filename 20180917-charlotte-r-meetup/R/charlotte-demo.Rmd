...
# do not use the development version of tibble
...
`tq_mutate()` and trading rules:

```{r}

bac %>% tq_mutate(adjusted, EMA)


bac_ema_ma <- bac %>%
  # exponential MA
  # (pass in extra args to EMA!)
  tq_mutate(adjusted, EMA, n = 100) %>%
  # MA
  mutate(MA = slide_dbl(adjusted, mean, .size = 100))


slice(bac_ema_ma, 95:105)
...
`tq_transmute()` can return a variable number of rows.
